...
export default function SpotInfoPanel({
  ...
  const { canModerate, canContribute } = useAuth();

  const [activeTab, setActiveTab] = useState<Tab>("overview");
  const [placeData, setPlaceData] = useState<PlaceData | null>(spot.placeData || null);
  const [loadingPlace, setLoadingPlace] = useState(!spot.placeData);
  ...
  }, [onClose, isChronicleModalOpen]);

  // Fetch place data from server-side API route if not already provided
  useEffect(() => {
    if (spot.placeData) {
      setPlaceData(spot.placeData);
      setLoadingPlace(false);
      return;
    }
    if (!spot.title) return;

    setLoadingPlace(true);
    setPlaceData(null);

    const query = `${spot.title}, ${spot.address || spot.zone || "Kolkata"}`;
  ...
  }, [spot.title, spot.address, spot.zone, spot.placeData]);

  const photoUrl = placeData?.photos?.[0]?.url;

  const handleShare = () => {
    if (navigator.share) {
      navigator.share({ title: spot.title, text: spot.description, url: window.location.href }).catch(() => {});
    } else {
  ...
  const handleCopyCoords = () => {
    const text = `${spot.lat.toFixed(6)}, ${spot.lng.toFixed(6)}`;
    navigator.clipboard.writeText(text);
    setCopiedCoords(true);
    setTimeout(() => setCopiedCoords(false), 2000);
  };

  // Structured data for HeritageChronicleModal
  const chronicleData = {
    name: spot.title,
    ...
    historicalNames: [
      {
        name: spot.title,
        validFrom: "Heritage Era",
        validUntil: "Present",
        explanation: `${spot.title} is an iconic ${spot.category} landmark documented in ${spot.zone}. ${
          spot.address ? `Located at ${spot.address}.` : ""
        }`,
        source: spot.contributorName ? `Archived by ${spot.contributorName}` : "Tilottoma Heritage Archive",
      },
    ],
    sources: [
      {
        title: `${spot.title} Archival Record`,
        author: spot.contributorName || "Kolkata Heritage Registry",
        publisher: "Tilottoma Living Archive",
        url: placeData?.url || undefined,
      },
    ],
    tags: [
      spot.category.toLowerCase().replace(/[^a-z0-9]+/g, "-"),
      spot.zone.toLowerCase().replace(/[^a-z0-9]+/g, "-"),
      "heritage-landmark",
      "kolkata",
    ],
    contributorName: spot.contributorName || "Tilottoma Community",
  };

  const directionsUrl =
    placeData?.url ||
    `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(
      spot.title + " " + (spot.address || spot.zone || "Kolkata")
    )}`;
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="w-full h-48 sm:h-52 md:h-56 bg-zinc-900 relative">
              {loadingPlace ? (
                ...
                <div className="w-full h-full flex flex-col items-center justify-center text-zinc-600 bg-linear-to-br from-zinc-900 via-zinc-950 to-zinc-900 p-6 text-center">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5">
                <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                  <span className="text-[9px] font-mono font-semibold uppercase tracking-wider px-2 py-0.5 rounded bg-black/60 backdrop-blur-md text-zinc-200 border border-white/15">
                    {spot.isGooglePoi ? "GOOGLE POI • KOLKATA" : "HERITAGE LANDMARK • KOLKATA"}
                  </span>
                  {spot.zone && (
                  ...
                  {spot.category && (
                    <span className="text-[9px] font-mono font-medium uppercase px-2 py-0.5 rounded bg-zinc-800/80 text-zinc-200 backdrop-blur-md border border-white/10">
                      {spot.category}
                    </span>
                  )}
                </div>
            ...
            <div className="flex px-5 py-2.5 gap-2 overflow-x-auto scrollbar-none">
              ...
              {(canModerate || canContribute) && onEditSpot && !spot.isGooglePoi && (
                <button
                  type="button"
                  ...
                  title="Edit place details, address or historical folklore"
                >
            ...
            <div className="p-5 pb-8 space-y-5">
              ...
              {activeTab === "overview" && (
                <section className="space-y-5">
                  ...
                  <div className="p-4 rounded-xl bg-zinc-50/80 dark:bg-zinc-900/50 border border-zinc-200/80 dark:border-zinc-800 space-y-2.5">
                    ...
                    <p className="line-clamp-4 leading-relaxed text-xs sm:text-sm text-zinc-700 dark:text-zinc-300 font-sans">
                      {spot.description || "No detailed historical overview recorded yet."}
                    </p>
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {chronicleData.tags.map((tag, idx) => (
                      <span
                        key={idx}
                  ...
                  {/* Famous Places & Zone Recommendations directly below Brief */}
                  <div className="pt-2 border-t border-zinc-200/60 dark:border-zinc-800/60">
                    <ZoneRecommendations
                      streetId={spot.streetId || undefined}
                      streetName={spot.streetName || spot.title}
                      zone={spot.zone || "Central Kolkata"}
                    />
                  </div>

                  {/* Spatial Telemetry Card */}
                  <div className="p-4 rounded-xl bg-zinc-50/50 dark:bg-zinc-900/40 border border-zinc-200/80 dark:border-zinc-800 space-y-2.5">
                    <div className="flex items-center justify-between text-[11px] font-mono font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
                      <span>Spatial Coordinates</span>
                      <button
                        type="button"
                        onClick={handleCopyCoords}
                        className="text-[10px] text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1 cursor-pointer font-normal"
                      >
                        {copiedCoords ? (
                          <>
                    ...
                    <div className="flex items-center justify-between text-xs font-mono text-zinc-700 dark:text-zinc-300 bg-white dark:bg-zinc-900 p-2 rounded-lg border border-zinc-200/60 dark:border-zinc-800">
                      <span>Latitude: {spot.lat?.toFixed(6)}</span>
                      <span>Longitude: {spot.lng?.toFixed(6)}</span>
                    </div>
                  ...
                  <div className="p-4 rounded-xl bg-zinc-50/50 dark:bg-zinc-900/40 border border-zinc-200/80 dark:border-zinc-800">
                    <VerticalNameTimeline
                      currentName={spot.title}
                      historicalNames={chronicleData.historicalNames as any}
              ...
              {/* TAB 3: SPATIAL POINTS / RECOMMENDATIONS */}
                ...
                <section className="space-y-4">
                  <div>
                    ...
                    <div className="space-y-2">
                      {chronicleData.sources.map((source, i) => (
                        <div
                          key={i}
            ...
            {spot.contributorName ? (
              <div className="flex items-center gap-2 text-zinc-700 dark:text-zinc-300 font-medium">
                ...
                <span className="truncate text-xs">
                  Provenance: <strong>{spot.contributorName}</strong>
                </span>
              </div>
            ) : (
